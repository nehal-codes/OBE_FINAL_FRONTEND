...
const ProgramReports = () => {
  ...
  const fetchReportData = async () => {
    ...
    try {
      ...
      const response = await HOD_API.reports.getProgramReport(
        selectedProgram,
        token,
      );
      ...
      if (response.data.success) {
        ...
        if (response.data.data.courses?.length > 0) {
          setSelectedCoursesForRadar(
            response.data.data.courses.slice(0, 3).map((c) => c.id),
          );
        }
      } else {
  ...
  const fetchCourseDetails = async (courseId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await HOD_API.reports.getCourseContributions(
        courseId,
        token,
      );
  ...
  const exportToCSV = () => {
    ...
    const headers = [
      ...
      "Course Name",
      "Semester",
      ...reportData.pos.map((p) => p.code),
      ...reportData.psos.map((p) => p.code),
    ];
  ...
  const getPOContributions = () => {
    ...
    return reportData.pos.map((po) => {
      ...
      reportData.courses.forEach((course) => {
        const contribution =
          reportData.coursePOContributions[course.id]?.[po.id];
  ...
  const getRadarChartData = () => {
    if (!reportData?.pos || !reportData?.psos || !reportData?.courses)
      return [];
    ...
    const radarData = allOutcomes.map((outcome) => {
      ...
      selectedCoursesForRadar.forEach((courseId) => {
        ...
        if (course) {
          ...
          if (outcome.type === "PO") {
            value =
              reportData.coursePOContributions[courseId]?.[outcome.id]
                ?.averageLevel || 0;
          } else {
            value =
              reportData.coursePSOContributions[courseId]?.[outcome.id]
                ?.averageLevel || 0;
          }
  ...
  const toggleCourseForRadar = (courseId) => {
    setSelectedCoursesForRadar((prev) => {
      if (prev.includes(courseId)) {
        ...
        if (prev.length < 6) {
          // Limit to 6 courses max for readability
  ...
  const getOutcomeCoveragePercentage = (courseId) => {
    ...
    const courseCloDetails =
      reportData.cloDetails?.filter((clo) => clo.courseId === courseId) || [];
    ...
    courseCloDetails.forEach((clo) => {
      ...
      if (clo.poMappings && clo.poMappings.length > 0) {
        clo.poMappings.forEach((m) => {
          if (m.poCode && m.level > 0) {
            // ONLY count if level > 0
    ...
    const totalOutcomes =
      (reportData.pos?.length || 0) + (reportData.psos?.length || 0);
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">
            Please contact your administrator to add programs.
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">
            No report data found for the selected program.
          </p>
          <button
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-600 mt-1">
                    Analyze course contributions to Program Outcomes (POs) and
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-green-600 mt-2">
                  {reportData.courses?.length > 0
                    ? (
                        (reportData.courses?.filter((c) => c.clos?.length > 0)
                          .length /
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-orange-600 mt-2">
                  {reportData.cloDetails?.length > 0
                    ? (
                        (reportData.cloDetails?.filter(
                          (c) =>
                            // Check if CLO has any PO mappings with level > 0 OR any PSO mappings with level > 0
                            c.poMappings?.some((m) => m.level > 0) ||
                            c.psoMappings?.some((m) => m.level > 0),
                        ).length /
                          reportData.cloDetails?.length) *
                        100
                      ).toFixed(0)
                    : 0}
                  % Mapped
        ...
        <div className="space-y-6">
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      {selectedCoursesForRadar.map((courseId, index) => {
                        const course = reportData.courses.find(
                          (c) => c.id === courseId,
                        );
                          ...
                          return [
                            <div className="flex flex-col">
                              <span className="font-medium">
                                Level: {value.toFixed(2)} / 3
                              </span>
                              <span className="text-xs text-gray-500">
                                {formatAsPercentage(value)} attainment
                              </span>
                            </div>,
                            name,
                          ];
          ...
          {viewMode === "single-course" && (
            <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
              ...
              {selectedCourse &&
              reportData.coursePOContributions[selectedCourse] ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {reportData.psos.map((pso) => {
                        const contribution =
                          reportData.coursePSOContributions[selectedCourse]?.[
                            pso.id
                          ];
                        const value = contribution?.averageLevel || 0;
                        const percentage = (value / 3) * 100;
                                ...
                                <td className="px-4 py-2 whitespace-nowrap">
                                  <span
                                    className={`px-2 py-1 rounded-full text-xs font-medium ${
                                      clo.bloomLevel === "REMEMBER"
                                        ? "bg-red-100 text-red-800"
                                        : clo.bloomLevel === "UNDERSTAND"
                                          ? "bg-yellow-100 text-yellow-800"
                                          : clo.bloomLevel === "APPLY"
          ...
          {viewMode === "heatmap" && (
            <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
                      ...
                      {reportData.heatmapData.courses
                        ...
                        .map((course) => (
                          ...
                          >
                            ...
                            {reportData.pos?.map((po) => {
                              const value =
                                reportData.heatmapData.matrix[course.id]?.[
                                  po.id
                                ]?.value || 0;
                              const percentage = (value / 3) * 100;
                              return (
                                <td
                                  key={po.id}
                                  className="px-6 py-4 group relative"
                                >
              ...
              <p className="text-sm text-gray-500 mt-4 flex items-center gap-2">
                ...
                Click on any row to view detailed CLO contributions for that
                course
              </p>
            </div>
          )}
              ...
              <div className="lg:col-span-2 bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
                ...
                {reportData.courses?.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {reportData.courses
                      .sort(
                        (a, b) => (b.clos?.length || 0) - (a.clos?.length || 0),
                      )
                      .slice(0, 6)
                      .map((course) => {
                        // FIXED: Use course.id to get the correct coverage percentage
                        const coveragePercentage = getOutcomeCoveragePercentage(
                          course.id,
                        );
            ...
            <div className="p-6 border-b border-gray-200 flex justify-between items-center bg-gradient-to-r from-blue-600 to-purple-600">
              <div>
                ...
                <p className="text-blue-100 mt-1">
                  {courseDetails.course.code} • Semester{" "}
                  {courseDetails.course.semester} •{" "}
                  {courseDetails.course.credits} Credits
                </p>
              </div>
              <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {courseDetails.summary?.totalClos || 0}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-xl p-4">
                  <p className="text-sm text-gray-600">PO Mappings</p>
                  <p className="text-2xl font-bold text-gray-900">
                    {courseDetails.summary?.totalPoMappings || 0}
                  </p>
                  <p className="text-xs text-gray-500">
                    {courseDetails.summary?.totalClos &&
                    reportData.pos?.length > 0
                      ? (
                          (courseDetails.summary?.totalPoMappings /
                            (courseDetails.summary?.totalClos *
                              reportData.pos.length)) *
                          100
                        ).toFixed(0)
                      : 0}
                    % mapping density
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {courseDetails.summary?.totalPsoMappings || 0}
                  </p>
                  <p className="text-xs text-gray-500">
                    {courseDetails.summary?.totalClos &&
                    reportData.psos?.length > 0
                      ? (
                          (courseDetails.summary?.totalPsoMappings /
                            (courseDetails.summary?.totalClos *
                              reportData.psos.length)) *
                          100
                        ).toFixed(0)
                      : 0}
                    % mapping density
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <p
                    className={`text-2xl font-bold ${
                      courseDetails.summary?.avgPoLevel >= 2.5
                        ? "text-green-600"
                        : courseDetails.summary?.avgPoLevel >= 1.5
                          ? "text-yellow-600"
                          : "text-red-600"
                    }`}
                  >
                  ...
                  <p className="text-xs text-gray-500">
                    {((courseDetails.summary?.avgPoLevel / 3) * 100).toFixed(0)}
                    % attainment
                  </p>
                </div>
              </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
                      ...
                      {Object.keys(clo.poMappings || {}).length > 0 && (
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {Object.entries(clo.poMappings).map(
                              ([code, data]) => (
                                <div
                                  key={code}
                      ...
                      {Object.keys(clo.psoMappings || {}).length > 0 && (
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {Object.entries(clo.psoMappings).map(
                              ([code, data]) => (
                                <div
                                  key={code}
                                  className={`px-3 py-1.5 rounded-lg text-sm border ${
            ...
            <div className="p-6">
              ...
              <div className="space-y-3">
                {reportData.courses
                  ?.filter(
                    (course) =>
                      reportData.coursePOContributions?.[course.id]?.[
                        selectedPO.id
                      ]?.averageLevel > 0,
                  )
                  .map((course) => {
                    ...
                    const percentage = contribution
                      ? ((contribution.averageLevel / 3) * 100).toFixed(0)
                      : 0;
                    return (
                      <div
                        key={course.id}
                      ...
                      >
                        ...
                        <div
                          className={`px-3 py-1 rounded-lg font-medium ${
                            contribution?.averageLevel >= 2.5
                              ? "bg-green-100 text-green-800"
                              : contribution?.averageLevel >= 1.5
                                ? "bg-yellow-100 text-yellow-800"
                                : "bg-red-100 text-red-800"
                          }`}
                        >
