...
const CLOMapping = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const sortedPos = (poRes?.data?.pos || []).sort((a, b) =>
          (a.code || "").localeCompare(b.code || ""),
        );
        const sortedPsos = (poRes?.data?.psos || []).sort((a, b) =>
          (a.code || "").localeCompare(b.code || ""),
        );
        setPos(sortedPos);
        setPsos(sortedPsos);
  ...
  const submit = async () => {
    ...
    try {
      ...
      navigate(`/hod/courses/${actualCourseId}/clos`);
    } catch (error) {
      ...
      <div className="max-w-[1800px] mx-auto">
              ...
              <thead className="bg-gray-50">
                <tr>
                  ...
                  {pos.length > 0 && (
                    <th
                      colSpan={pos.length}
                      className="py-3 px-2 text-center border-b border-gray-300 bg-blue-50"
                    >
                  ...
                  {psos.length > 0 && (
                    <th
                      colSpan={psos.length}
                      className="py-3 px-2 text-center border-b border-gray-300 bg-green-50"
                    >
        ...
        <div className="mt-8 flex flex-col sm:flex-row justify-between gap-4">
          <div className="flex gap-3">
            <button
              onClick={() => navigate(`/hod/courses/${actualCourseId}/clos`)}
